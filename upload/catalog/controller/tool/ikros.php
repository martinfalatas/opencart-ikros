...
class ControllerToolIkros extends Controller {
    ...
    private function cronManager($product, $order, $invoice){
        $cron_statuses = $this->config->get('ikros_cron_status_id');
        if(isset($cron_statuses)){
            if ($product and in_array(1, $cron_statuses)){
                $this->download();
            }

            if ($order and in_array(2, $cron_statuses)){
                $this->exportOrders();
            }

            if ($invoice and in_array(3, $cron_statuses)){
                $this->exportInvoices();
            }
        }
    }

    private function cronPasswordChecking(){
        if(isset($this->request->get['password']) && ( $this->request->get['password'] == $this->config->get('ikros_cron_key') ) ){
            return true;
        }else{
            return false;
        }
    ...
    public function import_products(){
        if( $this->cronPasswordChecking() ){
            $this->cronManager(true, false, false);
        }
    ...
    public function export_orders(){
        if( $this->cronPasswordChecking() ){
            $this->cronManager(false, true, false);
        }
    ...
    public function export_invoices(){
        if( $this->cronPasswordChecking() ){
            $this->cronManager(false, false, true);
        }
    }
    ...
    public function export_documents(){
        if( $this->cronPasswordChecking() ){
            $this->cronManager(false, true, true);
        }
    }
    ...
    public function import_export(){
        if( $this->cronPasswordChecking() ){
            $this->cronManager(true, true, true);
        }
    }
    //------------------------------------------------------------------------------------------------------------------
